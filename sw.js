...
.catch(function(){ return caches.match(’/updates.json’); })
);
return;
}
...
caches.open(CACHE).then(function(c){
var now = Date.now();
// no-store = App fordert explizit frische Version (z.B. nach Update)
// SW-Cache überspringen, frisch laden und Cache aktualisieren
if(e.request.cache === ‘no-store’){
return fetch(new Request(’/index.html’,{cache:‘no-store’})).then(function(res){
...
return c.match(’/index.html’) || new Response(’<p>Bitte einmal online öffnen.</p>’,{headers:{‘Content-Type’:‘text/html’}});
});
}
...
if(cached && !expired){
// Aus Cache servieren - noch frisch genug
return cached;
}
